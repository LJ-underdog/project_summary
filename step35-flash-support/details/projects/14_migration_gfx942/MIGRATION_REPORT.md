...
> **UPDATE 2026-05-09**（by `tp2_verify_post_merge_wave/L31`）：
> 本节描述的 NEW-RC-3 working-tree patch 已于 ~2026-04-30 commit 化为 aiter `f06cdcca5`
> （commit message: `fix(moe): force per_1x128 fp8 blockscale to CK 2-stage on gfx942`，
> diff 与 §6.4 内嵌 patch byte-identical）。
> 当前 step35-flash-support reproduce 路径见顶层 `REPRODUCE.md §3.1 / §3.4`。
> 本节内容保留作 wave 14 (2026-04-26) 实施时点的 historical migration 描述。
...
> **UPDATE 2026-05-09**（by `tp2_verify_post_merge_wave/L31`）：
> 本节描述的"~~NEW-RC-3 working-tree dirty~~"状态对应 wave 14 (2026-04-26) 实施时点。
> NEW-RC-3 patch 已于 ~2026-04-30 commit 化为 aiter `f06cdcca5`，working tree 不再 dirty。
> 当前 reproduce commit 见顶层 `REPRODUCE.md §3.1 / §3.4`（aiter `f06cdcca5` 已在主线祖先链）。
